 ...
 * This allows the fast path to be used exclusively, saving code size and
 * removing the generic path logic from the build when it's not needed.
 */
#define WS2812_SPI_NON_8BIT_SYMBOL_CHECK(idx) \
	(DT_INST_PROP(idx, bits_per_symbol) != SPI_FRAME_BITS) ||
#define WS2812_SPI_ALL_SYMBOLS_ARE_8_BITS \
	!(DT_INST_FOREACH_STATUS_OKAY(WS2812_SPI_NON_8BIT_SYMBOL_CHECK) 0)
